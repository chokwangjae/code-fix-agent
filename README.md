...
- 현재 호스트 OS에서 실행할 수 없는 명시적 OS 전용 하네스만 조건부 통과
...
OS 전용 하네스는 `conditional_test_commands`에 명령과 실행 가능한 `host_os`를 함께 적는다. 값은 `macos`, `linux`, `windows`다. 현재 OS가 목록 밖이면 명령을 실행하지 않고 결과와 `tests_conditional_pass` event에 사유를 남긴 뒤 조건부 통과한다. 현재 OS에서 실행된 명령의 assertion 실패, timeout, 비정상 종료는 조건부 통과로 바꾸지 않는다.

```toml
conditional_test_commands = [
  { command = ["xcodebuild", "test"], host_os = ["macos"] },
  { command = ["pwsh", "-File", "scripts/test.ps1"], host_os = ["windows"] },
]
```
...
Codex는 `codex_timeout_seconds`, 하네스는 `harness_timeout_seconds`, 그 밖의 준비·Git 명령은 `command_timeout_seconds`를 적용한다. 각 제한을 넘기면 자식까지 포함한 프로세스 그룹을 종료한다. 최초 claim부터 7200초는 완료 목표이며 실패 시점이 아니다. 5400초부터 반복 실패 finding을 개별 처리로 분리하고, 6600초부터 독립 검증을 통과한 그룹을 먼저 commit·push한다. 목표를 넘기면 job의 `timing_status`를 `overdue`로 바꾸고 사유를 기록한 채 계속 처리한다. 기본 최대 시간인 14400초에 도달해야 재시도 없는 `failed`로 끝내며, 이때 worktree와 commit checkpoint를 제거하지 않는다. 이전 `job_timeout_seconds`는 호환을 위해 목표 시간으로 해석하지만 새 설정에는 `target_duration_seconds`를 사용한다.
